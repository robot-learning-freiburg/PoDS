...
__all__ = ["PoDS", "INS_EMBED_BRANCHES_REGISTRY", "build_ins_embed_branch"]
...
class PoDS(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        self.mode = cfg.MODEL.PoDS.MODE
        self.stuff_area = cfg.MODEL.PoDS.STUFF_AREA
        self.threshold = cfg.MODEL.PoDS.CENTER_THRESHOLD
        self.nms_kernel = cfg.MODEL.PoDS.NMS_KERNEL
        self.top_k = cfg.MODEL.PoDS.TOP_K_INSTANCE
        self.predict_instances = cfg.MODEL.PoDS.PREDICT_INSTANCES
        self.use_depthwise_separable_conv = cfg.MODEL.PoDS.USE_DEPTHWISE_SEPARABLE_CONV
        assert (
            cfg.MODEL.SEM_SEG_HEAD.USE_DEPTHWISE_SEPARABLE_CONV
            == cfg.MODEL.PoDS.USE_DEPTHWISE_SEPARABLE_CONV
        )
        self.size_divisibility = cfg.MODEL.PoDS.SIZE_DIVISIBILITY
        self.benchmark_network_speed = cfg.MODEL.PoDS.BENCHMARK_NETWORK_SPEED
...
class PoDSSemDecoder(nn.Module):
    """
    ...
    def __init__(
        ...
        *,
        ...
        assert self.decoder_only, "PoDSSemDecoder is expected to be only decoder"
...
@SEM_SEG_HEADS_REGISTRY.register()
class PoDSSemSegHead(PoDSSemDecoder):
    """
...
@SEM_SEG_HEADS_REGISTRY.register()
class PoDSInsDecoder(nn.Module):
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        assert self.decoder_only, "PoDSInsDecoder is expected to be only decoder"
...
class PoDSInsEmbedHead(PoDSInsDecoder):
    """
    A instance embedding head described in :paper:`PoDS`.
    """
...
class PoDSHead(nn.Module):
    def __init__(
        ...
        super(PoDSHead, self).__init__()
        self.pods_head = nn.Sequential(
            DepthwiseSeparableConv2d(
                in_channels,
                decoder_channels,
                norm1=norm,
